...
- **Four handoff bundles, each governing a different question (DECISIONS Q18, extended
  by Q52).** `/design-reference-v3/` (fourth handoff) is the source of truth for every
  screen a **v3 phase touches** — read `project/HeiTuva.dc.html` there before building it.
  `/design-reference-v2/` (third handoff) is the reference for every screen a v2 phase
  built and no v3 phase touches, **and it is the only bundle holding the splash and
  Bruksområder at all** — v3 was a ONE-FILE handoff, so those two surfaces stay v2's.
  ...
  `artifacts/reference-v2/` and `artifacts/reference-v3/`. `npm run verify:reference` renders
  **the target set only** (v3) and overwrites no older set — those are regenerated by name
  (`--bundle=v2`, `--bundle=v1`, `--bundle=legacy`, `--all`), because a baseline re-rendered
  under a newer Chromium silently stops being the thing its phases were judged against. What
  changed between bundles, screen by screen: `docs/v1/00-diff.md`, `docs/v2/00-diff.md` and
  `docs/v3/00-diff.md`.

  **A HANDOFF NEED NOT CONTAIN EVERY FILE THE PREVIOUS ONE DID, AND v3 DID NOT.** The
