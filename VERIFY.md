...
3. scripts/verify/reference.ts — opens the design files in Playwright, drives their
   ...
   Since DECISIONS Q18, extended by Q52, it renders THREE handoffs and overwrites none:
   ...
   /design-reference-v2/…/HeiTuva.dc.html   -> artifacts/reference-v2/<screen>.png
     the third handoff, the TARGET for every screen a v2 phase touches. It has a third
     prototype file (Bruksomrader) the earlier bundles do not; screens are declared per
     bundle so a v2-only screen is skipped rather than captured from the wrong page.
   A default run renders the TARGET set only. The older sets are regenerated by name
   (--bundle=v1, --bundle=legacy, --all) because re-rendering a baseline under a newer
   Chromium moves PNGs with no design change behind them.
...
/design-reference-v2/… for anything a v2 phase touches, /design-reference-v1/… for
screens v1 built and v2 does not touch, /design-reference/… for Phase 1–7 work as built.
