...
3. scripts/verify/reference.ts — opens the design files in Playwright, drives their
   ...
   /design-reference-v2/…/HeiTuva.dc.html   -> artifacts/reference-v2/<screen>.png
     ...
     IT IS ALSO THE ONLY SET HOLDING splash*.png AND bruksomrader.png, because v3 was a
     one-file handoff — for those two surfaces this is the live reference, not a record.
   /design-reference-v3/…/HeiTuva.dc.html   -> artifacts/reference-v3/<screen>.png
     the fourth handoff, the TARGET for every screen a v3 phase touches. ONE FILE: no
     splash, no Bruksomrader. Screens are declared per bundle (`since`), so a screen
     whose file this bundle lacks is skipped rather than captured from the wrong page.
   A default run renders the TARGET set only. The older sets are regenerated by name
   (--bundle=v2, --bundle=v1, --bundle=legacy, --all) because re-rendering a baseline
   under a newer Chromium moves PNGs with no design change behind them.
...
/design-reference-v3/… for anything a v3 phase touches, /design-reference-v2/… for
screens v2 built and v3 does not touch (and for the splash and Bruksområder, which v3
did not hand over at all), /design-reference-v1/… for screens v1 built and v2 does not
touch, /design-reference/… for Phase 1–7 work as built.
