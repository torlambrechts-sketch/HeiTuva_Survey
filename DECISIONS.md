...
| Q14 | MFA for administrators | **CONFIRMED: deferred — no MFA enforcement for now.** Do not build or enforce TOTP. The capability stays available in Supabase Auth; it is simply not required at login. **Re-enable trigger: before the first real organisation is onboarded, or before prod holds any real respondent data — whichever comes first.** | Removed on Tor's instruction to cut development friction while there are no customers and no real data. Recorded as a deferral with a named trigger rather than a silent removal, because it is a security control and the point of the register is that nothing lapses by default. Note that leaked-password protection is also currently off; both belong to the same pre-launch gate. |
