 ...
 *
 * DECISIONS Q14 (re-enabled in Phase 7) requires MFA for administrators, so
 * after `npm run seed:demo` every administrator screen is unreachable until
 * this has run. Local stack only — the secrets are throwaway and the file is
 * gitignored. Auth users survive a re-seed, so this needs to run once per
 * stack, not once per seed.
 */
...
import { MFA_SECRETS_FILE, clearMfaFactors, totpCode, type MfaSecrets } from '../tests/db/mfa'
...
async function main() {
  ...
  for (const persona of ADMINS) {
    ...
    // so a factor we cannot generate codes for is worse than none. Removed
    // through the admin API, not the user's own session: GoTrue refuses to
    // unenrol a verified factor — and to enrol a new one — from an aal1
    // session ("AAL2 required to enroll a new factor"), which is exactly the
    // session a fresh password sign-in has.
    await clearMfaFactors(email)
