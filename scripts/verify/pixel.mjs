...
for (const [label, img] of [['baseline', base], ['shot', shot]]) {
  if (x + w > img.width || y + h > img.height) {
    console.error(
      `region ${w}x${h} at (${x},${y}) does not fit the ${label} (${img.width}x${img.height})`,
    )
    process.exit(2)
  }
}
...
console.log(`${name}: region ${w}x${h} at (${x},${y}) of a ${base.width}x${base.height} screen`)
